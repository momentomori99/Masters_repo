...
def simulate_brunels_network(input_data=None, g_strength=4):

    print(f"Running network with g = {g_strength}")
    ...
    N_noise = 0 # number of noise neurons to each neuron in the network
    ...
    if input_data is not None:
        ...
        # Normalize to a list of scalar rates 
    ...
    if n_clusters > 0:
        ...
        for k in range(n_clusters):
            ...
            pi = PoissonInput(target=E_cluster, target_var="v", N=N_input, rate=rate_k, weight=input_weights)
            feature_inputs.append(pi)
...
if __name__ == "__main__":
    sample_input =np.array([300, 0, 0, 0]) 

    voltage_monitor_E, voltage_monitor_I, spike_monitor_E, spike_monitor_I, rate_monitor_E, rate_monitor_I = simulate_brunels_network(input_data=sample_input, g_strength=10)
    ...
    plt.savefig("sample_input_random2.png", dpi=300, bbox_inches='tight')
    plt.show()